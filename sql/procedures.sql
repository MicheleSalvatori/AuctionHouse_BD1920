...
DELIMITER $$
CREATE PROCEDURE db_prova.inserisci_offerta (IN CF VARCHAR(16), IN val_offerta VARCHAR(15), IN id VARCHAR(25))
BEGIN
	IF EXISTS (SELECT * FROM db_prova.aste_aperte WHERE aste_aperte.ID = id)
		THEN IF EXISTS (SELECT * FROM db_prova.aste_aperte WHERE val_offerta > aste_aperte.PREZZO and aste_aperte.ID = id)
			THEN INSERT INTO  db_prova.offerte VALUES(CF, NOW(), val_offerta, default, id);
