...
CREATE TRIGGER auction_house.check_valid_dataNascita BEFORE INSERT ON auction_house.Utente
	FOR EACH ROW
...
CREATE TRIGGER auction_house.check_valid_dataScadenza BEFORE INSERT ON auction_house.Utente
	FOR EACH ROW
	...
	CREATE TRIGGER auction_house.check_exist_tipo BEFORE INSERT ON auction_house.Oggetto
			FOR EACH ROW
			BEGIN
				DECLARE temp_int INT;
		        SELECT count(*) INTO temp_int FROM auction_house.Tipo_oggetto WHERE Nome_Oggetto = NEW.Tipo;
		        IF (temp_int = 0) THEN
		        SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = "Tipo oggetto non presente nel database";
		        END IF;
			END $$

	CREATE TRIGGER auction_house.check_scadenza_asta BEFORE INSERT ON auction_house.Oggetto
			FOR EACH ROW
		    BEGIN
		    DECLARE h INT;
